...
4. [ShopSync.InventoryService ve gRPC Bütünleşmesi](#4-shopsyncinventoryservice-ve-grpc-bütünleşmesi)
5. [Veri Kalıcılığı (Persistence): Neden MongoDB Replica Set?](#5-veri-kalıcılığı-persistence-neden-mongodb-replica-set)
6. [İki Kademeli Idempotency: Mükerrer İşlemlere Karşı Kesin Çözüm](#6-iki-kademeli-idempotency-mükerrer-işlemlere-karşı-kesin-çözüm)
...
8. [Arka Plan İşçileri (Background Workers) ve DLQ](#8-arka-plan-işçileri-background-workers-ve-dlq)
9. [OpenTelemetry ile Tam Kapsamlı Gözlemlenebilirlik](#9-opentelemetry-ile-tam-kapsamlı-gözlemlenebilirlik)
10. [🔥 K6 İle Performans ve Stres Testi Stratejisi (Core)](#10--k6-i̇le-performans-ve-stres-testi-stratejisi-core)
11. [Kod İncelemesi: Background Jobs ve DLQ İşleyişi](#11-kod-i̇ncelemesi-background-jobs-ve-dlq-i̇şleyişi)
12. [Kod İncelemesi: Altyapı ve Docker Compose](#12-kod-i̇ncelemesi-altyapı-ve-docker-compose)
...
- **Controllers Katmanı:** İstekleri alır, FluentValidation ile gelen JSON verisini denetler (örn: negatif ürün adedi varsa anında reddeder).
...
graph TD
    ...
    subgraph "ShopSync Core Backend (DDD & Mikroservis Ağı)"
        ...
        subgraph "📦 Order Service (DDD Bounded Context)"
            OrderControllers[Controllers<br/>Fluent Validation]
        ...
        subgraph "🏭 Inventory Service (gRPC Sunucusu)"
            InventoryGrpc[gRPC Services<br/>Port 5001]
        end
        ...
        InventoryGrpc -->|Atomic Updates ($inc)| InventoryDB
        OrderJobs <--> OrderDB
...
## 4. ShopSync.InventoryService ve gRPC Bütünleşmesi

Inventory (Envanter) servisi dış dünyadan tamamen soyutlanmış, yalnızca iç ağdan (Internal Docker Network) ulaşılan bir servistir.

- **gRPC Server ve Protobuf:** Protobuf (`.proto`) dosyası üzerinden kesin hatlarla çizilmiş kontratlar (Contract-First) sunar. Binary serilizasyon, JSON serilizasyonuna göre CPU'yu %40 oranında daha az yorar ve Payload boyutunu %60 küçültür. 
...
## 6. İki Kademeli Idempotency: Mükerrer İşlemlere Karşı Kesin Çözüm
...
graph TD
    ...
    B -->|CLOSED (Kapalı / Sağlıklı)| C[gRPC İsteği: Inventory.Reserve]
    B -->|OPEN (Açık / Kopuk)| H([Fail-Fast HTTP 503 Döner])
    B -->|HALF-OPEN (Yarı Açık)| I[Probing: 1 Test İsteğine İzin Ver]
    
    C --> D{Yanıt Başarılı mı?}
    D -->|Evet| E([İşleme Devam Et])
    D -->|Hayır (Timeout / 503)| F[Wait & Retry + Jitter]
...
## 10. 🔥 K6 İle Performans ve Stres Testi Stratejisi (Core)
...
## 11. Kod İncelemesi: Background Jobs ve DLQ İşleyişi
...
## 12. Kod İncelemesi: Altyapı ve Docker Compose
